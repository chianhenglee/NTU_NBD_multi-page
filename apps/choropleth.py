...
df = pd.read_csv('apps/choropleth_data/Test_data.csv')


######## Loading Essential Data for plotting polygons of counties and cities #######
# Get counties name list
with open('apps/choropleth_data/counties/counties_list.txt') as f:
    Clist=f.read().splitlines()

# Get centroid of polygons correspond to Clist order
with open('apps/choropleth_data/counties/counties_centroid.txt') as f2:
...
i=0
for c in Clist:
    with open('apps/choropleth_data/counties/'+c+'.json') as f:
...
layout = html.Div([
    ...
    dcc.Link('Back to index page', href='/',style={'marginLeft':'20px'}),html.Br(),
    ...
    html.Div([
        dcc.Graph(
            id='my_graph_choropleth_barchart',
            figure={
                'data': [
                    {'x': [], 'y': []}
                    ],
...
@app.callback(
    Output('my_graph_choropleth', 'figure'),
    [Input('submit-button', 'n_clicks')],
    [State('my_year_symbol', 'value'),
    State('my_age_symbol','value'),
    State('my_gender_symbol', 'value'),
    State('my_graph_choropleth','figure')])

def update_graph_choropleth(n_clicks, year, age, gender, figure):
...
@app.callback(
    Output('my_graph_choropleth_barchart', 'figure'),
    [Input('submit-button', 'n_clicks')],
    [State('my_year_symbol', 'value'),
    State('my_age_symbol','value'),
    State('my_gender_symbol', 'value'),
    State('my_graph_choropleth_barchart','figure')])
def update_graph_choropleth_barchart(n_clicks, year, age, gender, figure):
